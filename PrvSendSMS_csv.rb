...
#*********
    ...
    api_password    = 'byjmut#fegkoD&7hunvi'
    api_token       = 'Sv4tSC3CbjxMjMqTUa0kzrBd'
    api_recipients  = [
        "+32476651363",     #Gilbert
        "+32473725032"      #Bernadette
    ]
    api_message     = ''
    api_msghdr      = "*****\n"
...
#*********
    ...
    #Authenticate
    #++++++++++++
    _com.step("Authenticate")
    # Create the HTTP objects
    uri             = URI.parse("https://api.smsenvoi.com/API/v1.0/REST/login")
    ...
    request.basic_auth 'cssghe@heintje.net', 'byjmut#fegkoD&7hunvi'
    ...
    if responsedata.code == "200"
        ...
        _com.step("user_key:#{user_key} ")
        _com.step("Session_key:#{session_key} ")
    else
        _com.step("Error::CODE:" + responsedata.code + ", MESSAGE:" + responsedata.body)
        _com.exit(program,"<Authenticate error>")
        exit 3
    end
    #
    #Main phase
    ...
    #Send message
    #++++++++++++
    uri = URI.parse("https://api.smsenvoi.com/API/v1.0/REST/sms")
    # Parameters
    payload =     {
            "message_type"              => "MND", 
            "message"                   => api_message, 
            "recipient"                 => api_recipients, 
    #        "sender"                    => api_sender, 
    #        "scheduled_delivery_time"   => api_time, 
    #        "order_id"                  => api_order, 
            "returnCredits"             => true,
    ...
    _com.step("REQUEST::#{request.body}")
    
    # Send the request & check response
    if _mode == 'E'     #<IF1>
        responsedata = http.request(request)
        if responsedata.code == "201"   #<IF2>
            response    = responsedata.body
            obj         = JSON.parse(response)
            _com.step("RESPONSE:#{obj}")
        else    #<IF2>
            _com.step("Error::CODE:" + responsedata.code + ", MESSAGE:" + responsedata.body)
